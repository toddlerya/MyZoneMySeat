...
def do_book(session_obj, seat_room, seat_num, seat_id, start, end):
    """
    执行预定动作
    :param session_obj:
    :param seat_room:
    ...
    session_obj.book_seat(seat_id=seat_id, room=seat_room, number=seat_num, start=start, end=end_time, date=end)
...
if __name__ == '__main__':
    ...
    if login_status:
        ...
        for _ in range(0, 120, 15):  # 从起始时间开始, 每间隔15分钟尝试一次任务, 直到两个小时为止
            start_time += 15
            ...
            for seat in goal_seats:
                ...
                book_form.append([h, seat_room, seat_num, seat_id_code, start_time, h.tomorrow_date])
            with ThreadPoolExecutor(thread_num) as executor:
                for each in book_form:
                    executor.map(do_book, each)
        # ====================== 调试代码 ==============================
        # 每次提交预定信息前要先访问一次"自助选座"获取"预定token", 否则会出现非法Invalid CSRF token错误
